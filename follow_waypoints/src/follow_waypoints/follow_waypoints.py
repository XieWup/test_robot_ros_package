...
class GetPath(State):
    ...
    def waypoint_mark_callback(self, waypoint_mark_msg):
	self.waypoint_mark_pose = waypoint_mark_msg.pose.pose.position.x
	rospy.loginfo(self.waypoint_mark_pose)
	waypoints.append(changePose(waypoint_mark_msg, "map"))
        # publish waypoint queue as pose array so that you can see them in rviz, etc.
        self.poseArray_publisher.publish(convert_PoseWithCovArray_to_PoseArray(waypoints))

    def path_ready_callback(self,path_ready_msg):
	...
	##self.goal_pub.publish(1)
		...
		x = pose.pose.pose.position.x
                ...
                if 'timeout exceeded' in e.message:
                    ...
                    raise e
            rospy.loginfo("Recieved new waypoint, x = "+str(x))
