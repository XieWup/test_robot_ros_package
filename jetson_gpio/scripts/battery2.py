...
x=serial.Serial('/dev/ttyUSB0',9600)
def faSong():   # 发送函数
    while True: # 循环发送数据
          
        myinput=b'\x7F\x10\x02\x06\x11\x58'    # 需要发送的十六进制数据
...
def jieShou():  # 接收函数
    while True: # 循环接收数据
        while x.inWaiting()>0:  # 当接收缓冲区中的数据不为零时，执行下面的代码
            myout=x.read(27) # 提取接收缓冲区中的前7个字节数
            #print (myout)
           ...
           # if checksum_high==myout[31] and checksum_low==myout[32]:
            #data=myout
            current=(myout[10]*255+myout[9])*0.1
            if current>0:
               state=1
            else:
               state=3
            rsoc_last=(myout[22]*255+myout[21])*0.1
            rsoc_all= (myout[24]*255+myout[23])*0.1
            rsoc = rsoc_last/rsoc_all
            print(state)  
            #return data
