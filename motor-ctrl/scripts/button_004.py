...
class simple_class:
  ...
  def __init__(self):
    ...
    self.sub5 = rospy.Subscriber("amcl_pose", PoseWithCovarianceStamped, self.callback5)
  ...
  def callback1(self,data):
    ...
    self.btn_state2=False      
    self.target = Pose(Point(0.819,-1.120, 0.000), Quaternion(0.000, 0.000, -0.704, 0.710))
    #取餐点
    self.goal = MoveBaseGoal()  
    self.goal.target_pose.pose = self.target
    self.goal.target_pose.header.frame_id = 'map'  
 ...
 #   self.staue=data.data
  ...
  def callback2(self,data):
    ...
    self.target = Pose(Point(0.692, -3.236, 0.000), Quaternion(0.000, 0.000, -0.968, 0.250))
    #送餐点
    self.goal = MoveBaseGoal()  
    self.goal.target_pose.pose = self.target
    self.goal.target_pose.header.frame_id = 'map'  
  ...
  def callback3(self,value) :
    ...
    if self.rsoc<=0.3 and self.current<0 and self.staue==False and  self.btn_state == False  :
      self.target = Pose(Point(0.000, 0.001, 0.000), Quaternion(0.000, 0.000, 0.013,0.999))
      #充电点
      self.goal = MoveBaseGoal()  
      self.goal.target_pose.pose = self.target
    ...
    if self.staue==True :
       ...
       if self.current<0 and self.stm==False and self.stme==False :
          self.linear_x=0
          self.angular_z=0.15
       if self.current<0 and self.stm==False and self.stme==True :
          self.linear_x=0
          self.angular_z=-0.15
       if self.current<0 and (self.stm==True or -0.5<=self.position_x<=-0.45):  #对射传感器
          self.linear_x=-0.02
          self.angular_z=0
          self.stme=False
       if  self.current<0 and  self.position_x<-0.55:
          self.linear_x=-0.01
          self.angular_z=0 
       if self.current>0   :
          self.linear_x=0
  ...
  def callback4(self,data):
    #print(data.data)
    self.stm=data.data
    if self.staue==True and self.stme==False and self.stm==False and self.position_x>-0.45:
       rospy.sleep(3)
       self.stme=True
